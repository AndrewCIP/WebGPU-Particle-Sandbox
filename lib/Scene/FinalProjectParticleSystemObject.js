...
const INPUT_TIME_INDEX = 8;
const INPUT_TIME_OFFSET_BYTES = INPUT_TIME_INDEX * Float32Array.BYTES_PER_ELEMENT;
...
export default class FinalProjectParticleSystemObject extends SceneObject {
  ...
  resetParticles() {
    const colorMode = Number.isFinite(this._inputData[9]) ? Math.max(0, Math.min(7, Math.floor(this._inputData[9]))) : 0;
  ...
  updateInput(input) {
    ...
    this._inputData[INPUT_TIME_INDEX] = now;
    this._inputData[9] = input.colorMode || 0;
  ...
  compute(pass) {
    ...
    const now = performance.now() / 1000.0;
    this._inputData[INPUT_TIME_INDEX] = now;
    this._device.queue.writeBuffer(this._inputBuffer, INPUT_TIME_OFFSET_BYTES, this._inputData, INPUT_TIME_INDEX, 1);
